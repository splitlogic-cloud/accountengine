...
export function evaluateRules(transaction: Transaction, rules: Rule[]): RuleMatch {
  const sorted = [...rules].sort((a, b) => a.priority - b.priority)
  for (const rule of sorted) {
    if (!rule.is_active) continue
    if (!rule.conditions || rule.conditions.length === 0) continue
    const allMatch = rule.conditions.every(cond => evaluateCondition(transaction, cond))
    if (allMatch) return { matched: true, rule, action: rule.action }
  }
...
function evaluateCondition(tx: Transaction, condition: RuleCondition): boolean {
  ...
  switch (condition.operator) {
    ...
    case 'between': {
      const n = parseFloat(val), lo = parseFloat(cmp), hi = parseFloat(condition.value2 ?? '0')
      return n >= lo && n <= hi
    }
    case 'in': return cmp.split(',').map(s => s.trim()).includes(val)
    case 'not_in': return !cmp.split(',').map(s => s.trim()).includes(val)
    default: return false
  }
}

function getFieldValue(tx: Transaction, field: RuleCondition['field']): string | number | null {
  switch (field) {
...
export function generateJournalLines(transaction: Transaction, rule: Rule) {
